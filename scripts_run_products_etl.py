...
def run_etl():
    ...
    for int_column in ['product_code', 'brand_id', 'category_id', 'colour_id', 'size_id', 'gender_id']:
        valid_df[int_column] = pd.to_numeric(valid_df[int_column], errors='coerce')

    valid_df['price'] = pd.to_numeric(valid_df['price'], errors='coerce')
    valid_df['active'] = valid_df['active'].apply(normalize_active)

    valid_df = valid_df.dropna(
        subset=['product_code', 'product_name', 'brand_id', 'category_id', 'colour_id', 'size_id', 'gender_id', 'price']
    )

    valid_df = valid_df[valid_df['price'] >= 0]

    for int_column in ['product_code', 'brand_id', 'category_id', 'colour_id', 'size_id', 'gender_id']:
        valid_df = valid_df[valid_df[int_column] > 0]
        valid_df[int_column] = valid_df[int_column].astype(int)
